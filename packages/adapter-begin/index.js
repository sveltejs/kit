...
const copy_contents = (source_directory, base_destination_directory, relativeTo = '.') => {
  let assets = [];
	glob('**/*', { cwd: source_directory, filesOnly: true }).forEach(file => {
    if (file[0] == '.') { return };
    copy_single(source_directory, base_destination_directory, file);
    assets.push(`/${relative(relativeTo, join(base_destination_directory, file))}`);
  });
  return assets
}
...
module.exports = async function builder({
  ...
  const static_assets = copy_contents('static', static_directory, static_directory);
  const client_assets = copy_contents(resolve(dir, 'client'), join(static_directory, '_app'), static_directory);
  ...
  child_process.execSync('npm install', { stdio:[0,1,2], cwd: lambda_directory });
  ...
  writeFileSync(join(server_directory, 'template.js'), `module.exports = ${JSON.stringify(appHtml)};`);
  const all_static_assets = JSON.stringify([ ...static_assets, ...client_assets ]);
  writeFileSync(join(server_directory, 'static_assets.js'), `module.exports = ${all_static_assets}`);
