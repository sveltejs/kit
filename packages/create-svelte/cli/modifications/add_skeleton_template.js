...
export default async function add_skeleton_template(cwd, yes) {
	if (yes) {
        ...
        copy_from_template_additions(cwd, { from: ['template-skeleton/index.svelte'], to: ['src/routes/index.svelte']});
        copy_from_template_additions(cwd, { from: ['template-skeleton/$layout.svelte'], to: ['src/routes/$layout.svelte']});
