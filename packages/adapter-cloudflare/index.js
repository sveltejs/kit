	...
	return {
		...
		async adapt(builder) {
			...
			try {
				...
				if (result.metafile) {
					writeMetaFile(dest, result.metafile);
				}

				printBundleStats(outfile);
			} catch (error) {
 ...
 */
function writeMetaFile(dest, metafile) {
	if (!metafile) {
		return;
	}

	try {
		writeFileSync(`${dest}/meta.json`, JSON.stringify(metafile));
		console.log(`Metafile written to ${dest}/meta.json`);
	} catch (error) {
		console.error('Failed to write metafile\n');
		console.error(error);
	}
}
 ...
 */
function printBundleStats(bundleFile) {
	try {
