...
export function strip_headers_rules(file_contents, patterns) {
	...
	for (const line of file_contents.split('\n')) {
		...
		if (content && /^\s/.test(line)) {
			...
			// blank line or comment — `parse_headers` ignores these without ending the
			// current block (indented header lines after them still belong to the
			// preceding pattern), so we must not reset `skip` here either. Otherwise a
			// blank line inside a stripped block would let subsequent header lines
			// survive as orphaned, malformed lines — and be duplicated in the merged
			// autogenerated section. The line itself is harmless to keep.
			lines.push(line);
		}
	}
