...
export function reset_states(import_meta_url) {
	if (!import_meta_url.match(/reset-states\/[+_]server\.js$/)) { // allow _server for build entrypoint
		throw new Error('must be called from reset-states/+server.js but was called from '+import_meta_url);
	}
