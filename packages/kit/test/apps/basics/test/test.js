...
test.describe.parallel('Routing', () => {
	...
	test('history index gets set on first render', async ({ javaScriptEnabled, page }) => {
		await page.goto('/routing/history/a');

		if (javaScriptEnabled) {
			const state = await page.evaluate('history.state');
			expect(state?.['sveltekit:index']).toBe(0);
		}
	});

	test('history index increases after navigating by clicking a link', async ({
		javaScriptEnabled,
		page,
		clicknav
	}) => {
	...
	test('history index increases after navigating by using goto', async ({
		javaScriptEnabled,
		app,
		page
	}) => {
		...
		if (javaScriptEnabled) {
			await app.goto('/routing/history/b');
			const state = await page.evaluate('history.state');
			expect(state?.['sveltekit:index']).toBe(1);
		}
	});

	test('history index stays after navigating by using goto with replaceState', async ({
		javaScriptEnabled,
		app,
		page
	}) => {
		...
		if (javaScriptEnabled) {
			await app.goto('/routing/history/b', { replaceState: true });
			const state = await page.evaluate('history.state');
			expect(state?.['sveltekit:index']).toBe(0);
		}
	});

	test('history index stays after fixing tralingSlash', async ({
		javaScriptEnabled,
		app,
		page
	}) => {
	...
	test('history index decreases after navigating back', async ({
		javaScriptEnabled,
		clicknav,
		app,
		page
		...
		if (javaScriptEnabled) {
			await clicknav('[href="/routing/history/b"]');
			await app.goto('/routing/history/c');
			await page.goBack();
			const state1 = await page.evaluate('history.state');
			expect(state1?.['sveltekit:index']).toBe(1);
			await clicknav('button');
			const state2 = await page.evaluate('history.state');
			expect(state2?.['sveltekit:index']).toBe(0);
			await clicknav('[href="/routing/history/b"]');
			const state3 = await page.evaluate('history.state');
			expect(state3?.['sveltekit:index']).toBe(1);
		}
	});

	test('history index survives a reload', async ({ javaScriptEnabled, clicknav, app, page }) => {
		await page.goto('/routing/history/a');
		...
		if (javaScriptEnabled) {
			...
			await app.goto('/routing/history/c');
			const state = await page.evaluate('history.state');
			expect(state?.['sveltekit:index']).toBe(2);
		}
	});
