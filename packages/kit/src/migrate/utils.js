 ...
 * @param {(content: string, match: RegExpMatchArray) => {content: string, todo?: string, file?: {name: string; content: string; todo?: string}}} fun File content and match in, new content and possibly a todo and new file out
 */
export function forEachFile(branch_hierarchy, file_matcher, fun) {
	...
	if (file && match) {
		...
		if (new_file) {
			fs.writeFileSync(path.join(branch_hierarchy.path, new_file.name), new_file.content, 'utf-8');
			if (new_file.todo) {
				todos.push(
