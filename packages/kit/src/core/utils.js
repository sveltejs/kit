...
/**
 * Gets the listening address of a Vite dev server. Otherwise, it throws an error
 * @param {ViteDevServer} server
 * @returns {string}
 */
export function get_address(server) {
	const address = server.resolvedUrls?.local?.[0];
	if (!address) throw new Error('Failed to determine Vite dev server address');
	return address;
}
