...
async function analyse({ manifest_data, env, vite_config_file }) {
	...
	try {
		...
		// close the dev server so its open handles (optimizer, sockets, etc.) don't
		// keep the forked worker's event loop alive — otherwise workers and their dev
		// servers accumulate across `vite build --watch` rebuilds
