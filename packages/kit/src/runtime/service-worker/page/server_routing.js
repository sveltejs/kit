...
/**
 * @param {import('types').SWRClientRoute} route
 * @param {URL} url
...
export function generate_route_object(route, url, manifest) {
	...
	const nodes = [...errors, ...layouts.map((l) => l?.[1]), leaf[1]]
		...
		.map((n) => `'${n}': () => ${create_client_import(manifest.client.nodes?.[n], url)}`)
		.join(',\n\t\t');
...
export async function resolve_route(resolved_path, url, manifest) {
	...
	for (const candidate of manifest.client.routes) {
...
/**
 * @param {import('types').SWRClientRoute | null} route
...
function create_css_import(route, url, manifest) {
	...
	for (const node of [...errors, ...layouts.map((l) => l?.[1]), leaf[1]]) {
		...
		const node_css = manifest.client.css?.[node];
	...
	return `${create_client_import(/** @type {string} */ (manifest.client.start), url)}.then(x => x.load_css([${css}]));`;
}
