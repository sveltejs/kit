...
import { generate_route_object } from './server_routing.js';
 ...
 *   manifest: import('types').SWRManifest;
 *   state: import('types').SWRState;
...
export async function render_response({
	...
	if (page_config.ssr && page_config.csr) {
		body += `\n\t\t\t${fetched
			.map((item) => serialize_data(item, resolve_opts.filterSerializedResponseHeaders, false))
			.join('\n\t\t\t')}`;
	}

	if (page_config.csr) {
		const route = manifest.client.routes?.find((r) => r.id === event.route.id) ?? null;
