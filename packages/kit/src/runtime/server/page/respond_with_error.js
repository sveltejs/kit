...
export async function respond_with_error({ request, options, state, $session, status, error }) {
	const default_layout = await options.manifest.nodes[0](); // 0 is always the root layout
	const default_error = await options.manifest.nodes[1](); // 1 is always the root error
