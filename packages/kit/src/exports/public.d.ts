 ...
 * (thrown by your code, or code it calls), and framework errors (generated by SvelteKit
 * itself, such as 404s).
 */
export type CaughtError =
	| {
			/** An expected error, thrown with the `error` helper */
			kind: 'expected';
			/** The error body, as passed to `error(...)` */
			error: App.Error;
	  }
	| {
			/** An unexpected error, thrown by your app without the `error` helper */
			kind: 'unexpected';
			/** The thrown value. Contents may be unsafe to expose to users */
			error: unknown;
	  }
	| {
			/** An error generated by SvelteKit itself, such as a 404 */
			kind: 'framework';
			/** The status and a safe description of the error */
...
export type HandleClientError = (
	input: CaughtError & { event: NavigationEvent }
) => MaybePromise<AppErrorWithOptionalDefaults | VoidIfNoRequiredAppErrorProperties>;
