...
function kit({ svelte_config, adapter }) {
		...
		config: {
			...
			async handler(config, config_env) {
				...
				if (is_build) {
					...
					manifest_data = sync.all(svelte_config, config_env.mode, env, root).manifest_data;
				} else {
					new_config.define = {
						...define,
	...
	const plugin_guard = {
		...
		load: {
			...
			handler(id) {
				...
				manifest_data ??= sync.all(svelte_config, vite_config_env.mode, env, root).manifest_data;
	...
	const plugin_compile = {
		...
		async configureServer(vite) {
			return await dev(vite, vite_config, svelte_config, () => remotes, env, root, adapter);
		},
