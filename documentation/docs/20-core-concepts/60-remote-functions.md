...
The `value()` of a field does _not_ reflect defaults provided as a second argument to `as` (as in `fields.title.as('text', '...')`). You can programmatically update a field (or a collection of fields) via the `set(...)` method:
