...
In the second case, we don't want to give the attacker any help, so SvelteKit will generate a generic [400 Bad Request](https://http.dog/400) response. Validation failures pass through the server [`handleError`](hooks#handleError) hook with `kind: 'validation'` and an `error` object containing `{ status, message, issues }`. You can use the issues to log the failure or customise the response:
...
Be thoughtful about exposing validation issues, as they may give an attacker useful information.
