...
> [!NOTE] Single-flight updates replace `form`'s automatic invalidation for that submission: if the handler calls `.refresh()`, `.set()` or `.reconnect()` on any query, or the client calls `submit().updates(...)` (even with no arguments), only those updates are applied.
...
> [!NOTE] If the client requests updates that the handler never passes to `requested`, nothing is refreshed. For `form`, the automatic invalidation is skipped too, since updates were requested explicitly.
