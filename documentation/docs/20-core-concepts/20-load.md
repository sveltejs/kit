...
A `load` function in a `+page.js` file runs both on the server and in the browser. If your `load` function should _always_ run on the server (because it accesses private environment variables or a database, for example then it would go in a `+page.server.js` instead.
