...
A `load` function in a `+page.js` file runs both on the server and in the browser. If your `load` function should _always_ run on the server (because it uses private environment variables, for example, or accesses a database) then it would go in a `+page.server.js` instead. Conversely, if your `load` function should always run on the client-side (ex: you're building a [single page app](single-page-apps)) then you can add `export const ssr = false` to your `+page.js` or `+layout.js`, this will allow your `load` function to run in the client only.
