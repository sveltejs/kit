...
`error` throws, making it more ergonomic to stop execution within nested logic. In this case it tells SvelteKit to set the response status code to 404 and render an [`+error.svelte`](routing#error) component, where `$page.error` is the object provided as the second argument to `error(...)`.
