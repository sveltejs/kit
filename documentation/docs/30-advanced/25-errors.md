...
SvelteKit distinguishes between expected errors, unexpected errors, errors generated by the framework itself, and validation errors from invalid [remote function](remote-functions) arguments. All are represented as simple `{ status: number, message: string }` objects by default. Whichever kind it is, every error passes through the [`handleError`](hooks#handleError) hook — which can log it, and customise it — before it is rendered.
