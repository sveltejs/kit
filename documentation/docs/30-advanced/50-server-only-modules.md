...
- Add `.server` to the filename, e.g. `server.js` or `secrets.server.ts`. This works for any file in the project directory.
- Place it in a `server` directory anywhere in your project except inside `src/routes` or the assets directory, e.g `src/lib/server/config.js` or `src/lib/data/server/user/profile.js`. (In SvelteKit 2, this only applied to the `src/lib` folder.)
