...
For example, if the user fills out a form, but before submitting:
- clicks a link, then hits the browser's back button
- use the browser refresh

The values they filled in will be lost. In cases where it's valuable to preserve that input, you can take a _snapshot_ of DOM state, which can then be restored if the user navigates back.
