...
- `'expected'` — thrown with [`error(...)`](@sveltejs-kit#error)
  - `error` is the error body, which matches [`App.Error`](types#Error)
  - defaults to the error body itself
- `'unexpected'` — thrown by your code, or code it calls
  - `error` is the thrown value, which may contain information unsafe to expose
  - defaults to `{ status: 500, message: 'Internal Error' }`
- `'framework'` — generated by SvelteKit, such as 404s, 405s and 413s
  - `error` is `{ status, message }`, where `message` is safe text like `Not Found`
  - defaults to that same `{ status, message }`
- `'validation'` (server only) — caused by a remote function argument that does not match its [Standard Schema](https://standardschema.dev/)
  - `error` is `{ status: 400, message: 'Bad Request', issues }`
  - defaults to `{ status: 400, message: 'Bad Request' }`; the issues are not exposed unless you return them
