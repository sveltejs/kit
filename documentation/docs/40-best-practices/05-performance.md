...
As an example, enabling [single page app (SPA) mode](single-page-apps) may be the single worst thing you can do to harm performance. When you load a page which has been generated with server-side rendering (SSR) or prerendering, the browser can start rendering the content as soon as it receives the initial response. With SPA mode, an empty page is generated, which fetches JavaScript, which ultimately loads and renders the page. This results in an extra network roundtrip before a single pixel can be displayed, which is disastrous for performance — especially for mobile users.
