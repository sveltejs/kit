...
- Enabling [single page app (SPA) mode](single-page-apps) is another example of a waterfall. When you load a page which has been generated with server-side rendering (SSR) or prerendering, the browser can start rendering the content as soon as it receives the initial response. With SPA mode, an empty page is generated, which fetches JavaScript, which ultimately loads and renders the page. This results in an extra network roundtrip before a single pixel can be displayed, which harms performance — especially for mobile users where the network may be slow and JS can't be parsed as fast due to slower hardware.
