...
- Enabling [single page app (SPA) mode](single-page-apps) will cause such waterfall. With SPA mode, an empty page is generated, which fetches JavaScript, which ultimately loads and renders the page. This results in an extra network roundtrip before a single pixel can be displayed.
