...
- `page` is a [readable store](https://svelte.dev/tutorial/readable-stores) whose value reflects the object passed to `load` functions — it contains `host`, `path`, `params` and `query`. See the [`page` section](#loading-input-page) above for more details.
...
Be sure to [`subscribe`](https://svelte.dev/docs#4_Prefix_stores_with_$_to_access_their_values) these stores before using in your components. (In `load` functions, you can use them directly.)
