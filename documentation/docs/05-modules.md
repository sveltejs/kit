...
- `onBeforeNavigate((navigationIntent)=>void)` — a navigation interceptor that triggers before we navigate to a new route. This is helpful if we want to conditionally prevent a navigation or lookup the upcoming url. Currently the onBeforeNavigate interceptor has to wrapped inside of a if(browser) check.
