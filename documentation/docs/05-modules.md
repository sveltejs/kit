...
- `invalidate(resource)` causes the `load` functions belonging to the currently active page and layouts to re-run if they used the resource in question. The `resource` argument can either be a URL that has requested with `fetch` or a custom identifier registered with `uses(resource)` inside the `load` function. `invalidate` returns a `Promise` that resolves when the page is subsequently updated.
