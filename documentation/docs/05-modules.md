...
- `invalidate(resource)` causes the `load` functions belonging to the currently active page and layouts to re-run if they used the resource in question. The `resource` argument can either be a URL that has been requested with `fetch` or a custom identifier registered with `uses(resource)` inside the `load` function. `invalidate` returns a `Promise` that resolves when the page is subsequently updated.
...
- `files` is a function with the type of `(filepath: string) => boolean`. When `true`, the file will be included in the service worker manifest
