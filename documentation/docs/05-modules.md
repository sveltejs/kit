...
- `onBeforeNavigate((navigationIntent) => void)` — a navigation interceptor that triggers before we navigate to a new route. This function exposes a `navigationIntent` that allows us to lookup the route we going to navigate to as well as a `navigationIntent.cancel()` method to prevent the navigation from completing.
