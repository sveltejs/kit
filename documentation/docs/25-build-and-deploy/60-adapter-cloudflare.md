...
To enable only the [Node.js AsyncLocalStorage](https://developers.cloudflare.com/workers/configuration/compatibility-flags/#nodejs-asynclocalstorage) API, add the `nodejs_als` compatibility flag in your wrangler config using the `compatibility_flags` array.
