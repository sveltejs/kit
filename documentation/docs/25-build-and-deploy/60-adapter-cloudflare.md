...
The `fallback` option allows you to generate a fallback page.
The Pages default behaviour on a 404 (when there is no function invocation) is to serve `index.html`.
To override this, you can generate a `404.html` which cloudflare will serve instead.

More info on fallback pages can be found on the [adapter-static documentation](/docs/adapter-static#spa-mode-add-fallback-page).
