...
You'll need to exclude the `_headers` and `_redirects` files from your service worker using the [`config.kit.serviceWorker.files`](configuration#serviceWorker).
