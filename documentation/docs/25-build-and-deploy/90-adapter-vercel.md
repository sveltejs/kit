...
A random token used to bypass the cached version of a route. Send the token in a `__prerender_bypass=<token>` cookie to force the asset to be re-validated. See [Vercel's Draft Mode documentation](https://vercel.com/docs/draft-mode?framework=sveltekit) for more details.
