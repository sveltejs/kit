...
- Bypass the cached version of a route. Send the token in a `__prerender_bypass=<token>` cookie to render the route at request time. See [Vercel's Draft Mode documentation](https://vercel.com/docs/draft-mode?framework=sveltekit) for more details.
- Re-validate the route by making a `GET` or `HEAD` request with a `x-prerender-revalidate: <token>` header. The next request will return a fresh response.
