...
Advanced Bun build settings can be supplied with `buildOptions`. The adapter currently accepts `sourcemap`, `minify`, `bytecode`, `banner`, `footer`, `drop`, `external`, `features`, `optimizeImports`, `splitting`, and `compile`. Production dependencies are external by default in non-compiled builds so that packages left external by Vite are not rebundled; use `external` to add other packages. Compiled executables bundle production dependencies by default. Code splitting is enabled by default; `splitting: false` bundles the server into a single file, which works around [`Bun.build` output path collisions](https://github.com/oven-sh/bun/issues/17674) on applications whose module graph produces identically-hashed chunks.
