...
Run [`npx sv add sveltekit-adapter`](/docs/cli/sveltekit-adapter) to set everything up, or install manually with `npm i -D @sveltejs/adapter-static` and add the adapter to your `svelte.config.js`:
